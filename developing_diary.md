...
```
不过发射机构由于闭环工作没有完成，现在整体控制逻辑还比较粗劣，建议在没有完成闭环功能之前不要进行任何测试或者使用，因为会出一些难以解释的bug

我最好在下次更新里面写一个遥控器的off_line_detect()，检测一下有没有数据输入，没有数据输入的话我就直接进入STOP

然后我发现有两个多的用户串口可以用来传数据，但是这两个串口要4pin线，而这边没有，所以我鸽了，之后把车子给他们，我之后要弄弄arm杯了，和这个傻吊车子告别几天欸嘿嘿

虽然有点寂寞不过我感觉给这东西加点小修小补差不多吧，比如把遥控器离线检测功能加进去

不对，除非后面我有空反正这段时间我是绝对不碰这堆代码了，arm杯和课内要紧